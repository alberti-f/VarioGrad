...
main <- function(args = NULL) {
    ...
    print(paste("Set up data objects", as.character(Sys.time())))
    # Generate the train and test data objects
    train.data <- load_data(args$H, args$g, args$scale, args$side, group = "train", values_path = args$train_values_path, covariates_path = args$train_covariates_path)
    test.data <- load_data(args$H, args$g, args$scale, args$side, group = "test", values_path = args$test_values_path, covariates_path = args$test_covariates_path)
    ...
    print(paste("SL", sl.ID, " #", args$sl.idx, "    Selecting vertices", as.character(Sys.time())))
    ...
    print(paste("SL", sl.ID, " #", args$sl.idx, "    Fitting and evaluating basic LK models", as.character(Sys.time())))
    LKfit <- fit_LK_model(train.sl$locations, train.sl$values, LKparams, covariates = train.sl$covariates)
    ...
    LKeval.avg <- evaluate_LK_model(LKfit, locations_new = locations.test.avg, values_new = test.sl$values, covariates_new = test.sl$covariates)

    # Run permutation test for basic model
    print(paste("SL", sl.ID, " #", args$sl.idx, "    Running permutation test", as.character(Sys.time())))
    ...
    for (i in 1:n.permutations) {
        ...
        LKeval.tmp <- evaluate_LK_model(LKfit, locations_new = null.sl$locations, values_new = null.sl$values, covariates_new = null.sl$covariates)
    ...
    print(paste("SL", sl.ID, " #", args$sl.idx, "    Permutation test completed", as.character(Sys.time())))


    # Fit and evaluate centered model
    print(paste("SL", sl.ID, " #", args$sl.idx, "    Fitting and evaluating centered LK models", as.character(Sys.time())))
    LKfit.c <- fit_LK_model(train.sl.c$locations, train.sl.c$values, LKparams, covariates = train.sl.c$covariates)
    ...
    LKeval.avg.c <- evaluate_LK_model(LKfit.c, locations_new = locations.test.avg, values_new = test.sl.c$values, covariates_new = test.sl.c$covariates)

    # Run permutation test for centered model
    print(paste("SL", sl.ID, " #", args$sl.idx, "    Running permutation test", as.character(Sys.time())))
    null.data <- test.data
    ...
    for (i in 1:n.permutations) {
        ...
        LKeval.tmp <- evaluate_LK_model(LKfit.c, locations_new = null.sl$locations, values_new = null.sl$values, covariates_new = null.sl$covariates)
    ...
    print(paste("SL", sl.ID, " #", args$sl.idx, "    Saving results", as.character(Sys.time())))
    ...
    print(paste("SL", sl.ID, " #", args$sl.idx, "    Results saved at:", outpath))

    print(paste("SL", sl.ID, " #", args$sl.idx, "    LK analysis script completed", as.character(Sys.time())))
}
